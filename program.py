...
class Program:
    """Stores static method to import and export csv files"""
    @staticmethod
    def import_csv_users(filepath):
        """Imports csv files that store information about users by use add methods in user subclasses.
           Subclass of new instances depends on the information in 'role' columns"""
    ...
    def import_csv_attendance(filepath):
        """Imports csv files that store information about attendance by use add method in attendance class"""
        with open(filepath) as source:
    ...
    def import_csv_assignment(filepath):
        """Imports csv files that store information about assignments by use add method in assignment class"""
        with open(filepath) as source:
    ...
    def import_csv_submission(filepath):
        """Imports csv files that store information about submissions by use add method in submission class"""
        with open(filepath) as source:
            submission_csv_list = csv.DictReader(source)
            for submission in submission_csv_list:
                Submission.add_submission(submission["content"], submission["date"],
                                          submission["assignment_title"], submission["student_name"],
                                          submission["points"])

    @staticmethod
    def export_csv_users(filepath):
        """Exports information about users to csv file"""
        with open(filepath, "w") as user_csv:
            ...
            for user in user_list:
                user_dict = {'name': user.get_name(), 'mail': user.get_mail(),
                             'password': user.get_password(), 'role': user.get_class_name().lower()}
                user_writer.writerow(user_dict)

    @staticmethod
    def export_csv_attendance(filepath):
        """Exports information about attendance to csv file"""
        with open(filepath, "w") as attendance_csv:
            ...
            for attendance in attendance_list:
                attendance_dict = {'student_name': attendance.get_student().get_name(),
                                   'date': attendance.get_date(), 'status': attendance.get_status()}
                attendance_writer.writerow(attendance_dict)

    @staticmethod
    def export_csv_assignment(filepath):
        """Exports information about assignment to csv file"""
        with open(filepath, "w") as assignment_csv:
            ...
            for assignment in assignment_list:
                assignment_dict = {'title': assignment.get_title(), 'content': assignment.get_content(),
                                   'due_date': assignment.get_due_date(), 'max_points': assignment.get_max_points()}
                assignment_writer.writerow(assignment_dict)

    @staticmethod
    def export_csv_submission(filepath):
        """Exports information about submission to csv file"""
        with open(filepath, "w") as submission_csv:
            ...
            for assignment in assignment_list:
                for submission in assignment.get_list_submission():
                    submission_dict = {'assignment_title': submission.get_assignment().get_title(),
                                       'student_name': submission.get_owner().get_name(),
                                       'content': submission.get_content(), 'date': submission.get_date(),
                                       'points': submission.get_points()}
                    submission_writer.writerow(submission_dict)

    @staticmethod
    def import_all_csv():
        """Collects all csv import methods"""
    ...
    @staticmethod
    def export_all_cvs():
        """Collects all csv export methods"""
        Program.export_csv_users("Data/user.csv")
        Program.export_csv_attendance("Data/attendance.csv")
        Program.export_csv_assignment("Data/assignment.csv")
        Program.export_csv_submission("Data/submission.csv")
